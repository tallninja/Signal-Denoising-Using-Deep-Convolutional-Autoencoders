...
class AudioMetricsException(Exception):
    pass
...
def llr(clean_speech, processed_speech, fs, used_for_composite=False, frameLen=0.03, overlap=0.75):
    ...
    if not used_for_composite:
        # this line is not in composite measure but in llr matlab implementation of loizou
...
def wss(clean_speech, processed_speech, fs, frameLen=0.03, overlap=0.75):
    ...
    for i in range(num_crit):
        ...
        crit_filter[i, :] = np.exp(-11 *
                                   (((j - np.floor(f0))/bw)**2) + norm_factor)
        crit_filter[i, :] = crit_filter[i, :]*(crit_filter[i, :] > min_factor)

    num_frames = len(clean_speech)/skiprate - \
        (winlength/skiprate)  # number of frames
    start = 1  # starting sample

    hannWin = 0.5*(1-np.cos(2*np.pi*np.arange(1, winlength+1)/(winlength+1)))
    scale = np.sqrt(1.0 / hannWin.sum()**2)

    f, t, Zxx = stft(clean_speech[0:int(num_frames)*skiprate+int(winlength-skiprate)], fs=fs, window=hannWin, nperseg=winlength,
                     noverlap=winlength-skiprate, nfft=n_fft, detrend=False, return_onesided=True, boundary=None, padded=False)
    clean_spec = np.power(np.abs(Zxx)/scale, 2)
    clean_spec = clean_spec[:-1, :]

    f, t, Zxx = stft(processed_speech[0:int(num_frames)*skiprate+int(winlength-skiprate)], fs=fs, window=hannWin, nperseg=winlength,
                     noverlap=winlength-skiprate, nfft=n_fft, detrend=False, return_onesided=True, boundary=None, padded=False)
...
def pesq(clean_speech, processed_speech, fs, force_resample=True):
    ...
    if fs == 8000:
        ...
        pesq_mos = 46607/14945 - \
            (2000*np.log(1/(mos_lqo/4 - 999/4000) - 1))/2989
    elif fs == 16000:
        ...
        raise ValueError(
            'Sampling rate for PESQ must be either 8 kHz or 16 kHz')
